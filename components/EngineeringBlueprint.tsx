...
const ZONES: ZoneDetail[] = [
  ...
  },
  {
    ...
    highlight: 'آمن 100% على الأطعمة ومعدات الطهي ومطابق لاشتراطات السلامة الصحية'
  },
  {
    id: 'drainage',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 space-y-2 text-right">
            <div className="text-xs font-bold text-slate-700 pb-2 px-1">اختر القطاع لعرض خطة الرش:</div>
            {ZONES.map((zone) => {
              ...
              return (
                <button
                  ...
                  className={`w-full p-4 rounded-2xl border text-right transition-all flex items-center justify-between gap-3 cursor-pointer ${
                    isSelected
                      ? 'bg-emerald-50 border-emerald-600 text-slate-950 shadow-sm font-bold'
                      : 'bg-slate-50 border-slate-200 text-slate-800 hover:bg-slate-100 hover:border-slate-300 font-semibold'
                  }`}
                >
                  <div>
                    <div className="text-xs text-emerald-800 font-bold font-mono mb-0.5">{zone.zone}</div>
                    <div className="text-sm font-black text-slate-900">{zone.name}</div>
                  </div>
                  <div
          ...
          <div className="lg:col-span-8">
            <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm text-right">
              ...
              <div className="mb-5">
                <div className="text-xs font-bold text-slate-700 mb-2">الآفات المستهدفة في هذا النطاق:</div>
                <div className="flex flex-wrap gap-2">
                  {activeZone.pests.map((pest, idx) => (
                    <span
                      key={idx}
                      className="px-3 py-1 bg-white border border-slate-300 text-slate-900 text-xs font-bold rounded-lg shadow-xs"
                    >
                      {pest}
                    </span>
                  ))}
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="bg-white border border-slate-200 rounded-2xl p-3.5 shadow-xs">
                    <span className="text-slate-600 block mb-1 font-semibold">المبيد الكيميائي المصرح SFDA:</span>
                    <span className="text-emerald-900 font-black">{activeZone.chemical}</span>
                  </div>

                  <div className="bg-white border border-slate-200 rounded-2xl p-3.5 shadow-xs">
                    <span className="text-slate-600 block mb-1 font-semibold">القيمة المضافة والأمان:</span>
                    <span className="text-slate-900 font-bold">{activeZone.highlight}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-200 flex flex-wrap items-center justify-between gap-3">
                ...
                <a
                  href="tel:0558141870"
                  className="px-5 py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs rounded-xl transition flex items-center gap-2 shadow-sm active:scale-95"
                >
