      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {filteredReviews.map((rev) => (
            <div
              key={rev.id}
              className="bg-slate-50 border border-slate-200/80 rounded-3xl p-5 text-right flex flex-col justify-between shadow-sm hover:shadow-md transition"
            >
              <div>
                {/* Review Header */}
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h4 className="font-bold text-xs sm:text-sm text-slate-900">{rev.author}</h4>
                ...
                <div className="flex items-center gap-1 mb-2">
                  ...
                  <span className="text-[11px] text-slate-400 mr-2">{rev.date}</span>
                </div>

                {/* Service Tag */}
                <div className="inline-block bg-white text-emerald-800 border border-emerald-200 text-[10px] font-bold px-2 py-0.5 rounded-md mb-2">
                  {rev.serviceUsed}
                </div>
